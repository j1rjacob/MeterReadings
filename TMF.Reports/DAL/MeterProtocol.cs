...
{
    public class MeterProtocol : BaseDAL
    {
        public MeterProtocol()
        {
            SQL_DELETE = "[REPORT METERPROTOCOL_DEL]";
            SQL_GET = "[REPORT METERPROTOCOL_GET]";
            SQL_GET_LIST = "[REPORT METERPROTOCOL_LST]";
            SQL_INSERT = "[REPORT METERPROTOCOL_INS]";
            SQL_UPDATE = "[REPORT METERPROTOCOL_UPD]";
            PARM_ID = "@Id";
        }
        protected void SetInfo(out Model.MeterProtocol info, SqlDataReader reader)
        {
            try
            {
                info = new Model.MeterProtocol();
        ...
        public override IInfo GetRecord(SmartDB dbInstance, object Id)
        {
            ...
            array[0].Value = Id;
            Model.MeterProtocol bizObject = null;
            IInfo result;
            try
            {
        ...
        {
            var list = new List<Model.MeterProtocol>();
            IInfo result;
            try
            {
                bool transactionControl = dbInstance.TransactionControl;
        ...
        {
            string cmdText = "[REPORT METERPROTOCOL_LST_BYDESCRIPTION]";
            SqlParameter[] array = new SqlParameter[]
            {
