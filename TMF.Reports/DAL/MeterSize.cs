...
{
    public class MeterSize : BaseDAL
    {
        public MeterSize()
        {
            SQL_DELETE = "[REPORT METERSIZE_DEL]";
            SQL_GET = "[REPORT METERSIZE_GET]";
            SQL_GET_LIST = "[REPORT METERSIZE_LST]";
            SQL_INSERT = "[REPORT METERSIZE_INS]";
            SQL_UPDATE = "[REPORT METERSIZE_UPD]";
            PARM_ID = "@Id";
        }
        protected void SetInfo(out Model.MeterSize info, SqlDataReader reader)
        {
            try
            {
                info = new Model.MeterSize();
        ...
        public override IInfo GetRecord(SmartDB dbInstance, object Id)
        {
            ...
            array[0].Value = Id;
            Model.MeterSize bizObject = null;
            IInfo result;
            try
            {
        ...
        {
            var list = new List<Model.MeterSize>();
            IInfo result;
            try
            {
                bool transactionControl = dbInstance.TransactionControl;
        ...
        {
            string cmdText = "[REPORT METERSIZE_LST_BYDESCRIPTION]";
            SqlParameter[] array = new SqlParameter[]
            {
